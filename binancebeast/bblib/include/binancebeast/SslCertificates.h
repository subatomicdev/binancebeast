#ifndef BINANCEBEAST_SSLCERTIFICATES_H
#define BINANCEBEAST_SSLCERTIFICATES_H
...
namespace bblib
{
    ...
    /* 
        ...
        This is not for production code, from the Beast docs:
    ...
    */
    ...
    inline void load_test_certificates(ssl::context& ctx, boost::system::error_code& ec)
    {
    ...
    inline void load_root_certificates(ssl::context& ctx, const std::filesystem::path& path, boost::system::error_code& ec)
    {
        // TODO sanity check for size of file?
        std::vector<char> buffer;
        buffer.resize(std::filesystem::file_size(path));

        std::ifstream fs {path, std::ios_base::in};
        fs.read(buffer.data(), std::filesystem::file_size(path));

        ctx.add_certificate_authority(boost::asio::buffer(buffer.data(), buffer.size()), ec);
    }
}
